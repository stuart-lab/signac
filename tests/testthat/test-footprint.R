...
test_that("Footprint works", {
  ...
  expect_warning(test_atac <- AddMotifs(atac_small, genome, pwm, verbose=FALSE))
  test_atac <- Footprint(object=test_atac, 
                         motif.name = names(pwm),
                         compute.expected = FALSE, 
                         genome=genome)
  test_df <- GetFootprintData(test_atac, features = c("MA0031.1"))
