...
chromatinassay <- CreateChromatinAssayObject(counts = counts, genome = "hg19")
```
...
To create a Seurat object from scratch that contains a `ChromatinAssay` rather 
than a standard `Assay`, we use the `CreateSignacObject()` function. This is
similar to the `CreateSeuratObject()` function defined in Seurat, but adds
additional parameters to initialize the `ChromatinAssay` object.

```{r}
# create a Seurat object containing a ChromatinAssay
object <- CreateSignacObject(counts = counts, genome = "hg19")
```
...
object[["peaks"]] <- CreateChromatinAssayObject(counts = counts, genome = "hg19")
```
...
data in specific slots. This includes the `Fragments`, `Motifs`, and
`Annotation` functions. For example, to get or set gene annotation data we can
use the `Annotation()` getter and `Annotation<-` setter functions:
...
The `Fragments()` and `Motifs()` functions are demonstrated in other sections
below.
...
avoids the need to alter fragement files on disk.
...
`fragments` parameter in `CreateChromatinAssayObject()`, or pass the path to a
...
These parameters are also exposed in the `CreateSignacObject()` function.
For example:

```{r}
object <- CreateSignacObject(counts = counts, genome = "hg19", fragments = frag.path)
```
...
`ChromatinAssay`, rather than the as they appear in the fragment file itself.
...
[Bioconductor](https://bioconductor.org/packages/release/data/annotation/html/JASPAR2018.html)
