...
knitr::opts_knit$set(root.dir = "../../../mito_data")
```
...
We now import the mitochondrial variant data identified by 2 tools - 
...
informative by `MQuad`. Bi-alleloc genotyping of the chrM DNA was performed using
...
### MQuad-specific variant(s)

We now investigate the distinct variant(s) by plotting them with `FeaturePlot()`.
...
We can also visualize the distinct variant(s) identified by MQuad on the 
...
We investigate the common and unique variants identified using a venn diagram.
...
# plot venn
p5 <- ggVennDiagram(x = list(mgatk = mgatk_variants, mquad = mquad_variants),
                   set_size = 3, label_size = 3)
p5 + NoLegend() + coord_fixed(clip = "off")
```
