...
```{r message=FALSE, warning=FALSE}
mouse_brain <- qs2::qs_read("adult_mouse_brain.qs2")
mouse_brain
```
...
```{r message=FALSE, warning=FALSE}
# load the latest JASPAR motifs from AnnotationHub
ah <- AnnotationHub()
jaspar_path <- ah[["AH121720"]]
sq24 <- RSQLite::dbConnect(RSQLite::SQLite(), jaspar_path)
...
interneurons and use hypergeometric tests to assess the probability of
observing each motif at the given frequency by chance, comparing with a
...
top.da.peak <- rownames(da_peaks[da_peaks$p_val_adj < 0.05, ])
```
...
meta.feature <- mouse_brain[["peaks"]][[]]
...
```{r}
enriched.motifs <- FindMotifs(mouse_brain, features = top.da.peak)
```
