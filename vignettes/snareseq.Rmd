...
snare[["ATAC"]] <- CreateGRangesAssay(
  counts = atac[, colnames(snare)],
  fragments = fragments
)
...
VlnPlot(
  snare,
  features = c(
    "nCount_RNA", "nCount_ATAC", "TSS_enrichment", "Nucleosome_signal"
  ),
  pt.size = 0.1,
  ncol = 4
)
...
```{r}
DimPlot(snare, group.by = "predicted.id", label = TRUE, repel = TRUE, reduction = "umap.rna")
```
...
```{r include=FALSE}
qs2::qs_save(object = snare, file = "snare.qs2", nthreads = 4)
```
