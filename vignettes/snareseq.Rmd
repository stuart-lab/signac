...
[Sinto](https://timoast.github.io/sinto/). This fragment file can be downloaded
here: 
...
fragments <- "/home/stuartt/data/snare-seq/fragments_AdBrain.bed.gz"

# remove this once we have the proper fragment file made
colnames(atac) <- substr(x = colnames(atac), start = 5, stop = 16)
colnames(rna) <- substr(x = colnames(rna), start = 5, stop = 16)
...
```{r}
snare$high.tss <- ifelse(snare$TSS.enrichment > 2, 'High', 'Low')
TSSPlot(snare, group.by = 'high.tss')
```
...
DefaultAssay(snare) <- "ATAC"
CoveragePlot(snare, region = "chr2-22620000-22700000", features = "Gad2")
```
