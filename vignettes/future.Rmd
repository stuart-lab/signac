...
Here we demonstrate parallelization of the `FeatureMatrix` function and show some benchmark results to get a sense for the amout of speedup you might expect.
...
to `multicore`, `multiprocessor`, or `multisession` to get parallel processing, and set the 
...
for (i in seq_len(nrep)) {
  start <- Sys.time()
  fmat <- FeatureMatrix(fragments = fragments, features = peaks, cells = cells)
...
library(ggplot2)
results <- read.table("/home/stuartt/github/chrom/bldignore/timings.tsv", header = TRUE)
