...
knitr::opts_knit$set(root.dir = "../../vignette_data/GSE129785")
```
...
bone.cicero <- make_cicero_cds(
  cds = bone.cds, reduced_coordinates = reducedDims(bone.cds)$UMAP
)
```
...
We can add the co-accessible links found by Cicero to the `ChromatinAssay5`
...
in the `ChromatinAssay5`, and add this to the object using the `Links<-`
assignment function.

```{r}
links <- ConnectionsToLinks(conns = conns, ccans = ccans)
Links(bone, key = 'cicero') <- links
```
...
CoveragePlot(bone, region = "chr1:40189344-40252549", links = "cicero")
```

```{r include=FALSE}
qs2::qs_save(object = bone, file = "cd34.qs2", nthreads = 4)
```
