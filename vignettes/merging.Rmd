...
```{r message=FALSE, warning=FALSE}
pbmc500 <- CreateSignacObject(
  counts = pbmc500.counts,
  fragments = frags.500
)

pbmc1k <- CreateSignacObject(
  counts = pbmc1k.counts,
  fragments = frags.1k
)

pbmc5k <- CreateSignacObject(
  counts = pbmc5k.counts,
  fragments = frags.5k
)

pbmc10k <- CreateSignacObject(
  counts = pbmc10k.counts,
  fragments = frags.10k
)
```
...
# create objects
pbmc500 <- CreateSignacObject(counts = counts.500, sep = c(":", "-"), min.features = 500)
pbmc1k <- CreateSignacObject(counts = counts.1k, sep = c(":", "-"), min.features = 500)
pbmc5k <- CreateSignacObject(counts = counts.5k, sep = c(":", "-"), min.features = 500)
pbmc10k <- CreateSignacObject(counts = counts.10k, sep = c(":", "-"), min.features = 1000)
