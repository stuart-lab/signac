...
gr <- GRanges(
  seqnames = "chr1",
  ranges = IRanges(start = c(20, 70, 300), end = c(120, 200, 400))
)
gr
```
...
the objects. Alternatively, for human data, we could use the set of universal
scATAC-seq features recently released by our lab: [REMO](https://github.com/stuart-lab/REMO.v1.GRCh38).
...
md.10k <- md.10k[md.10k$passed_filters > 1000, ]
...
```{r message=FALSE, warning=FALSE}
pbmc500_assay <- CreateGRangesAssay(pbmc500.counts, fragments = frags.500)
pbmc500 <- CreateSeuratObject(pbmc500_assay, assay = "ATAC", meta.data = md.500)

pbmc1k_assay <- CreateGRangesAssay(pbmc1k.counts, fragments = frags.1k)
pbmc1k <- CreateSeuratObject(pbmc1k_assay, assay = "ATAC", meta.data = md.1k)

pbmc5k_assay <- CreateGRangesAssay(pbmc5k.counts, fragments = frags.5k)
pbmc5k <- CreateSeuratObject(pbmc5k_assay, assay = "ATAC", meta.data = md.5k)

pbmc10k_assay <- CreateGRangesAssay(pbmc10k.counts, fragments = frags.10k)
...
The object data is still stored as separate layers. To run downstream steps such
as dimension reduction with SVD, we need to first join the layers:
...
qs2::qs_save(object = combined, file = "pbmc_combined.qs2", nthreads = 4)
```
...
resulting merged count matrix **will not be accurate**.

The `merge` function will consider peaks that are not identical to be different
features entirely, even if they contain overlapping genomic ranges. This is why
we recommend to always create a consensus peak set and quantify those peaks
consistently across all the datasets to be merged.
...
counts.500 <- Read10X_h5(
  filename = "pbmc500/atac_pbmc_500_nextgem_filtered_peak_bc_matrix.h5"
)
counts.1k <- Read10X_h5(
  filename = "pbmc1k/atac_pbmc_1k_nextgem_filtered_peak_bc_matrix.h5"
)
counts.5k <- Read10X_h5(
  filename = "pbmc5k/atac_pbmc_5k_nextgem_filtered_peak_bc_matrix.h5"
)
counts.10k <- Read10X_h5(
  filename = "pbmc10k/atac_pbmc_10k_nextgem_filtered_peak_bc_matrix.h5"
)

# create objects
pbmc500_assay <- CreateGRangesAssay(counts = counts.500, min.features = 500)
pbmc500 <- CreateSeuratObject(pbmc500_assay, assay = "peaks")
pbmc1k_assay <- CreateGRangesAssay(counts = counts.1k, min.features = 500)
pbmc1k <- CreateSeuratObject(pbmc1k_assay, assay = "peaks")
pbmc5k_assay <- CreateGRangesAssay(counts = counts.5k, min.features = 500)
pbmc5k <- CreateSeuratObject(pbmc5k_assay, assay = "peaks")
pbmc10k_assay <- CreateGRangesAssay(counts = counts.10k, min.features = 1000)
