...
## How can I update my object from v1 to v2?

To update a `ChromatinAssay`-class assay from Signac v1 to a `GRangesAssay` for
use with Signac v2, you can use the `as.GRangesAssay()` function. For example:

```{r eval=FALSE, include=TRUE}
# example Seurat object containing a ChromatinAssay assay called "peaks"
obj <- readRDS("object.rds")

# update the peaks assay
obj[["peaks"]] <- as.GRangesAssay(obj[["peaks"]])
```
...
## Why do underscores ("_") in my feature names automatically get replaced with dashes ("-")?

Unfortunately, this is a constraint from Seurat as it reserves underscores for
delimiters between an object key and feature name. You may run into this
problem if using custom feature names, some NCBI Refseq annotations for non-model 
organisms, or non-UCSC formatted annotations. In these cases, you might want to
change the feature names to not use any underscores ("_") or dashes ("-"), or 
change the parsing mechanism upstream to find the appropriate start-end positions, 
because Signac uses dashes ("-") to determine chromosome start and end position.
