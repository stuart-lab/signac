...
pbmc.multi <- qs2::qs_read("multiomic/pbmc_multiomic.qs2")

# load the pre-processed atac data
pbmc.atac <- qs2::qs_read("pbmc_vignette/pbmc.qs2")
```
...
pbmc.atac[["ATAC"]] <- CreateGRangesAssay(
  counts = counts,
  fragments = Fragments(pbmc.atac)
)
...
p2 <- DimPlot(
  ...
  group.by = "predicted.id",
  label = TRUE,
  repel = TRUE) + NoLegend() + ggtitle("Query")
...
```{r include=FALSE}
qs2::qs_save(object = pbmc.atac, file = "pbmc_atac_integration.qs2", nthreads = 4)
```
