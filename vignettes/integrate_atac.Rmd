...
**Note that this functionality currently requires the development version of Seurat.
This can be installed from GitHub, see the installation instructions
[here](https://satijalab.org/seurat/articles/install.html#install-the-development-version-of-seurat-1)**
...
# process the hg38-mapped ATAC data
# first count fragments per cell
# fragpath <- "~/data/10x_atac/pbmc_10k_hg38/fragments.tsv.gz"
fragpath <- "~/data/10x_atac/pbmc10k_v2/atac_pbmc_10k_nextgem_fragments.tsv.gz"
...
quantify the mutliome peaks in the ATAC dataset to ensure that there are common features
...
(projecting each dataset into the other's LSI space) by setting `reduction="rlsi"`.
...
multimodal reference, and enabled cell type labels to be transfered from reference
