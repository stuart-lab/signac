...
```{r message=FALSE, warning=FALSE}
gr <- strsplit(rownames(mtx), "_")
gr <- GRanges(seqnames <- sapply(gr, `[[`, 1), ranges = IRanges(start = as.numeric(sapply(gr, `[[`, 2)), end = as.numeric(sapply(gr, `[[`, 3))))

bone_assay <- CreateGRangesAssay(
  counts = mtx,
  ranges = gr,
  min.cells = 5,
...
cluster_names <- c(
  "HSC", "MEP", "CMP-BMP", "LMPP", "CLP", "Pro-B", "Pre-B", "GMP",
...
VlnPlot(
  object = bone,
  features = c("nCount_ATAC", "TSS_enrichment", "Nucleosome_signal"),
  pt.size = 0.1,
  ncol = 3
)
```
...
hsc <- intersect(hsc, colnames(bone))
```
